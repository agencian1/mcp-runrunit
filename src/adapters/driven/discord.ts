...
function getToken(): string {
  const token =
    process.env.BOT_RUNRUNIT_REPORT?.trim() || process.env.BOT_RUNRUNIT_REPORT_PRIVATE_KEY?.trim();
  if (!token) {
    throw new Error(
...
async function discordFetch<T>(path: string, options: DiscordFetchOptions = {}): Promise<T> {
...
export async function createChannel(params: CreateChannelParams): Promise<DiscordChannel> {
  ...
  const channel = await discordFetch<DiscordChannel>(`/guilds/${params.guildId}/channels`, {
    method: 'POST',
    body,
  });
  return channel;
