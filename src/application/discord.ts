...
export async function createChannel(params: CreateChannelParams): Promise<DiscordChannel> {
...
export async function getOrCreateChannelForClient(params: {
  ...
  const guildId = params.guildId ?? (await getDefaultGuildId());
  const channelName = resolveChannelNameForClient(params.clientId, params.clientName);

  const channels = await discordApi.listGuildChannels(guildId);
  const textChannels = channels.filter((c) => c.type === 0);
  const existing = textChannels.find((c) => c.name?.toLowerCase() === channelName.toLowerCase());
  if (existing) {
    return {
  ...
  const created = await discordApi.createChannel({
    name: channelName,
    guildId,
    topic: params.clientName ? `Runrun.it client: ${params.clientName}` : undefined,
  });
  return {
...
function resolveChannelNameForClient(clientId?: number | string, clientName?: string): string {
  if (clientName?.trim()) {
    return clientName
      .trim()
      .replace(/[^a-z0-9-_]/gi, '-')
