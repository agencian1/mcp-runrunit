...
async function getAssigneeId(_body: CreateTaskBody) {
...
async function appendTaskDescription(taskId: number, incoming: string): Promise<void> {
  ...
  const existing = typeof current?.description === 'string' ? current.description.trimEnd() : '';
  const merged = existing.length === 0 ? trimmedIncoming : `${existing}\n\n${trimmedIncoming}`;
