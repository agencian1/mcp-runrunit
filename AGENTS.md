...
- `npm start` / `npm run dev` load the repository `.env` via `node --env-file=.env`; the stdio MCP entry Cursor typically uses does not, so variables such as `GITHUB_*` expected at runtime must be injected by the MCP host (or a launcher that adds `--env-file`) rather than assuming the repo `.env` is read automatically.
